...
quickVerify[f1_, f2_, wait_: 10, maxwait_: 20, explicit_: False, 
    ...
    status1 = If[StringQ[t],
        ...
        Null],
        If[t==False,
            If[t == False, "Failed with FullSimplify. Trying PossibleZeroQ...", Null],
        Null]
    ];
    (* Exit calculation if it takes longer than maxwait *)
    
    t = If[StringQ[t] || t == False, 
    TimeConstrained[PossibleZeroQ[n1 == n2], maxwait - wait, "Failed"], t];
    status2 = If[StringQ[t],
        If[explicit,(*Only prints out text if explicit flagged as True*)
            StringForm["Failed to determine solution within maxwait of ``s. Exited.", maxwait],
            If[t == False, "Failed with PossibleZeroQ.", Null]
        Null],
    Null];
    
    output = If[SameQ[t, True], 
        {status1, status2, t // VerificationTest},(*Just output test result if passed*)
        {status1, status2, t // VerificationTest, "First Function", 
            If[fullSimplify, f1 // FullSimplify, f1],
            "Second Function", 
            If[fullSimplify, f2 // FullSimplify, f2]}(*Outputs numerator, denominator if test fails*)
    ];
