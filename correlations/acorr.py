...
from clibs import iterateSymmCartesian
...
def acorrMapped(op_samples, sep_map, sep, mean, norm = 1.0, tol=1e-7, counts=False):
    ...
    while front < n:            # keep going until exhausted sep_mapay
        new_front = abs(sep_map[front]-sep_map[front-1]) > tol  # check if front value is a new pair
...
def acorrMapped2(op_samples, sep_map, sep, mean, norm = 1.0, tol=1e-7, counts=False):
    """This is a second attempt at the acorr mapped algorithm because I 
    ballsed up the first :)
    
    Theoretical basis is identical to acorrMapped
    ...
    if sep == 0: 
        # note that in HMC we don't have any repeated elements 
        # so 0 seps can only be the array on itself
        result = acorr(op_samples, mean, separation=0, norm = norm)
        if counts: result = (result, n)
        return result
    ...
    ans     = 0.0
    count   = 0
    iterateSymmCartesian(
        ans, count,
        sep_map,
        op_samples,
        mean, tol, n
    )
    # cannot calculate if no pairs
    result = ans/(count*norm) if count > 0.0 else np.nan 
    if counts: result = (result, count)
    return result
