...
class Constant_Energy(object):
    ...
    def lattice(self, p0, x0, step_sample, step_sizes):
        """Lattice variant
    ...
    def continuum(self, p0, x0, step_sample, step_sizes):
        """Lattice variant
        ...
        # check a number of step sizes and trajectory lengths
        for n_steps_i, step_size_i in zip(np.ravel(step_sample), np.ravel(step_sizes)):
            ...
            # obtain new dynamics and resultant hamiltonian
            self.dynamics.newPaths()
            pf, xf = self.dynamics.integrate(copy(p0), copy(x0))
            h_new = self.pot.hamiltonian(pf, xf)
...
class Reversibility(object):
    ...
    def continuum(self, p0, x0):
        """Checks the integrator is reversible
        by running and then reversing the integration and 
        verifying the same point in phase space
        
        Required Inputs
            p0          :: np array :: momentum
...
if __name__ == '__main__':
    ...
    test.lattice(p0, x0, step_sample, step_sizes)
    ...
    test = Constant_Energy(pot, dynamics)
    utils.newTest(test.id)
    test.continuum(p0, x0, step_sample, step_sizes)
    
    test = Reversibility(pot, dynamics)
    utils.newTest(test.id)
    test.continuum(p0, x0)